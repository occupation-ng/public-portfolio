...
function clickerit() {
    ...
    el.style.zIndex = "300"
    //el.innerHTML="➫" //wrong shadow, good for down right pointing rotate(30deg)
    el.innerHTML = "➬"
    el.style.transform = "rotate(210deg)"
    document.body.appendChild(el)
}
clickerit()
/*
 // DEV, overrides click events and creates a blue dot where you clicked.
 window.addEventListener("click",function(event) {
   let el = document.createElement("div")
   el.style.width="10px"
   el.style.height="10px"
   el.style.background="blue"
   el.style.left=event.x-5+"px"
   el.style.top=event.y-5+"px"
...
window.addEventListener("keydown", function(event) {
    ...
    if (event.ctrlKey && event.key === "ArrowLeft") {
        ...
        el.style.left = (parseInt(el.style.left) - mv) + "px"
    }
    ...
    if (event.ctrlKey && event.key === "ArrowUp") {
        event.preventDefault();
        let el = document.getElementById("clicker")
        el.style.top = (parseInt(el.style.top) - mv) + "px"
    }
    ...
    if (event.ctrlKey && event.key === "Enter") {
        ...
        window.dispatchEvent(evt); // target: null
        //document.dispatchEvent(evt);
        console.log(window.dispatchEvent(evt))
        //click
    }
...
/*
for multple event listeners on a single target
  also overrides all div events, ie a single window.addeventlistner("click") overiding 'on-click: (fn [e] ..)'s
